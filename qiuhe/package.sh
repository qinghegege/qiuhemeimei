...
MODULE_ZIP="$OUTPUT_DIR/qiuhe-module-${VERSION}.zip"
...
echo "  已生成: release/qiuhe-module-${VERSION}.zip"

# --- Shell 脚本独立包 ---
echo "[2/2] 打包 Shell 脚本独立包..."

SCRIPT_ZIP="$OUTPUT_DIR/qiuhe-script-${VERSION}.zip"
rm -f "$SCRIPT_ZIP"
...
echo "  已生成: release/qiuhe-script-${VERSION}.zip"

echo ""
echo "打包完成!"
echo ""
echo "文件:"
echo "  release/qiuhe-module-${VERSION}.zip   → Magisk/KSU 模块 (刷入安装)"
echo "  release/qiuhe-script-${VERSION}.zip   → Shell 脚本 (解压到手机, MT 管理器执行)"
echo ""
echo "移动端使用:"
echo "  1. 将 .zip 文件传到手机"
